...
export class CreateGroupComponent implements OnInit, AfterViewInit {
  ...
  private getSubjectsBySpecialiaztion(id: number){
    this.subjectService.getAllForSpecialization(id).subscribe(
      res=>{
        ...
        this.subjectsSource.paginator = this.subjectsPaginator;
        this.subjectsSource.sort = this.subjectsSort;
      }
    );
  }
  ...
  public setTeacherForSubject(subject: Subject, teacher: Teacher){
    ...
    if(this.subjectTeacherGroup.filter(x=>x.subject.id == subject.id)){
      this.subjectTeacherGroup = this.subjectTeacherGroup.filter(x=>x.subject.id != subject.id);
    }
